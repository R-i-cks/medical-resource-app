...
# --------------------- ADICIONAR TRADUÇOES DO MINIDICIONARIO ------------------------------


f = open("conceitos_relacoes_e_sinonimos.json","r",encoding='utf-8')
dicionario = json.load(f)

f1 = open("traducoes.json","r",encoding='utf-8')
traducoes = json.load(f1)

dicionario_lower = {k.lower(): v for k, v in dicionario.items()}

for traducao in traducoes['IP']:
    if traducao.lower() in dicionario_lower:
        print(traducao)
        if 'pt' not in dicionario_lower[traducao.lower()]:
            dicionario_lower[traducao.lower()]['pt'] = traducoes['IP'][traducao].split(' / ')
            dicionario_lower[traducao.lower()]['fonte(s)'].append('Minidicionário do Cardiologista')


file_out= open(r"conceitos_rel_sin_trad.json","w",encoding='utf-8')
json.dump(dicionario_lower,file_out,ensure_ascii=False,indent=4)
file_out.close()
